...
namespace Nop.Web.Components
{
    public class HomepageBestSellersViewComponent : NopViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int? productThumbPictureSize, bool showAll = false)
        {
            ...
            var report = await _staticCacheManager.GetAsync(
                ...
                async () => await (await _orderReportService.BestSellersReportAsync(
                    storeId: store.Id,
                    pageSize: showAll ? int.MaxValue : _catalogSettings.NumberOfBestsellersOnHomepage)).ToListAsync());
